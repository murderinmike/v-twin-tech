...
def carregar_sistema_ia():
    try:
        ...
        p = ChatPromptTemplate.from_messages([
            ("system", "You are a Master V-Twin Motorcycle Mechanic. Answer professionally and clearly based on the context."),
...
st.markdown("""
    ...
    .stTextInput input { background-color: #1E1E1E !important; color: white !important; }
    
    .html-custom-btn-solid, .html-giant-btn { background-color: #FF6600 !important; color: #FFFFFF !important; font-weight: bold; }
    .html-custom-btn-vazado { border: 2px solid #FF6600; color: #FF6600; }
    
    .main-btn-container { display: flex; justify-content: center; width: 100%; margin: 30px 0; }
    .html-home-motor, .html-brain-motor { display: flex; justify-content: center; width: 100%; }
    </style>
...
def enviar_mensagem_chat():
    ...
    if sistema_ia is None:
        st.session_state["chat_history"].append({"role": "assistant", "content": "❌ Erro ao carregar o sistema de IA."})
        return
